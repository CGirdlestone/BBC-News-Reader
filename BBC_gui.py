...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        """Creates the buttons linking to each article and a generic 'quit' button."""
    ...
    def save_article(self):
        """Method to save the current article as a text file, including the date when the file was saved."""

        # Set today's date 
        today = str(date.today())

        # Create a file with filename 'YYYYMMDD-headline'. Strip any double or single quotes from the headlines.
        # Write the date and article to the file.
        with open('{}-{}.txt'.format(today.replace('-',''),self.headline.strip(["'",'"'])),'a+') as f:
            f.write('Date saved: {}\n'.format(today))
            f.write(self.article_body)

    def headline_buttons(self):
        """Create buttons for each headline. Each button is given the command to open the headline's article."""

        # Create the headline buttons. 'headline = headline' is used to ensure the button refers to the correct
        # headline and not only the final one. Pack the buttons
        self.buttons = [tk.Button(self.root, text=headline, command= lambda headline = headline: self.open_article(headline)) for headline in self.news_reader.headlines]
        for button in self.buttons:
            button.pack(fill=tk.BOTH, expand=1, padx=2, pady=2)

    def create_article_window(self, headline):
        """Creates a new window for the article body and a frame to house the text and scrollbar."""
    ...
    def create_article_widgets(self, article_body):
        ...
        # Create the text widget and insert the article text. Configure the text widget so its y-value reflects that of the scrollbar.
    ...
    def open_article(self, article_headline):
        """Gets the body of text for the article. The article headline is required to allow the BBCNewsReader's load_new_tree
            method to be called"""

        # loop through the headlines until the correct article is found, then get the new html tree and get the main body of text
